...
for i = timelist
    ...
    if ~isempty(ls_pts)
        ...
        for ov = 1:length(oneov)-1
            index = oneov(ov);
            rowov = unpnts(index,:);
            onemat = [onemat; rowov];
        end
        for ov = 1:length(twoov)-1
            index = twoov(ov);
            rowov = unpnts(index,:);
            twomat = [twomat; rowov];
        end
        for ov = 1:length(threeov)-1
            index = threeov(ov);
            rowov = unpnts(index,:);
            threemat = [threemat; rowov];
        end
        for ov = 1:length(fourov)-1
            index = fourov(ov);
            rowov = unpnts(index,:);
            fourmat = [fourmat; rowov];
        end
        for ov = 1:length(fiveov)-1
            index = fiveov(ov);
            rowov = unpnts(index,:);
            fivemat = [fivemat; rowov];
        end
        for ov = 1:length(sixov)-1
            index = sixov(ov);
            rowov = unpnts(index,:);
            sixmat = [sixmat; rowov];
        end
        ...
        [row654,~] = size([fourmat;fivemat;sixmat]);

        [row65,~] = size([fivemat;sixmat]);
